...
// ================================================================
// WHATSAPP BOT v36.8
// Original phone detection + LID fallback + diagnostics
// ================================================================
...
// Fallback hardcoded admin LIDs (used only if phone detection fails)
...
// ORIGINAL working version — checks all @s.whatsapp.net fields
...
// Admin detection — tries phone first, then LID
...
// COMMAND LIST
// ================================================================
const COMMAND_LIST = `🥖 *BreadBot v36.8*
...
async function handleMessage(msg) {
  ...
  // Save raw message for diagnostics
...
async function handleAdminCommand(text, chatJid, msg) {
  ...
  switch (cmd) {
    ...
    case 'rawmsg': {
      ...
      const s = JSON.stringify(lastRawMsg, null, 2).slice(0, 3000);
...
const PANEL_HTML = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>BreadBot v36.8</title>
...
<h1>🥖 BreadBot v36.8</h1><div class="sub">Admin phone: <b id="adminPhone">—</b> · Admin LIDs: <b id="adminLids">—</b> · Scraper: <b id="scraperUrl">—</b></div>
<div class="grid">
