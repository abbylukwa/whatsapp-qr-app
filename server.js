// ════════════════════════════════════════════════════════════════════════════════
//  WHATSAPP BOT v16.0
//  2 Target Groups: Music + Movies | Multi-AI | 5 Download
//  v16 FIX: Admin detection — was reading r.jid (phone) instead of r.lid (actual LID)
//  v15 FIX: Invidious proxy endpoint | Piped proxyUrl | Smart cascade DL | Consent cookie
// ════════════════════════════════════════════════════════════════════════════════
...
const AI_ROUTES = {
  group_reply: ['groq', 'zai'],           // Shallow group replies to members
  group_decision: ['groq'],                 // Content posting decisions
  inbox_chat: ['dadgpt', 'uncensored'],     // Deep inbox conversations
  member_benefit: ['dadgpt', 'groq', 'zai'] // Member benefit AIs
};
...
async function aiDecideContent(channelKey, weather) {
  ...
  const musicPrompt = 'You are a music curator for a Zimbabwean WhatsApp group. Pick the BEST music genre for right now.\n\nCurrent conditions:\n- Time: ' + hour + ':00 (CAT)\n- Weather: ' + desc + ', ' + temp + 'C in Harare\n- Mood: ' + mood + '\n- Current Movies block: ' + block.name + '\n\nAvailable genres: ' + Object.keys(MUSIC_GENRES).join(', ') + '\n\nReply with ONLY the genre name, nothing else.';
  const tvPrompt = 'You are a content curator for a Zimbabwean WhatsApp Movies/Comedy group. Pick the BEST content category for right now.\n\nCurrent conditions:\n- Time: ' + hour + ':00 (CAT)\n- Weather: ' + desc + ', ' + temp + 'C in Harare\n- Mood: ' + mood + '\n- Movies block: ' + block.name + ' (theme: ' + block.theme + ')\n- Block categories: ' + (block.categories || []).join(', ') + '\n\nAvailable categories: ' + Object.keys(MOVIES_QUERIES).join(', ') + '\n\nReply with ONLY the category name, nothing else.';
  const prompt = channelKey === 'music' ? musicPrompt : tvPrompt;

  try {
    const reply = await callAi(provider, [
      { role: 'system', content: 'You are a content curator. Reply with ONLY one word - the genre or category name. No explanation, no punctuation.' },
      { role: 'user', content: prompt }
    ], 20);
    // Extract just the word
    const picked = reply.trim().toLowerCase().replace(/[^a-z_]/g, '');
    console.log('[AI-Decide] ' + channelKey + ': AI picked "' + picked + '" (weather=' + desc + ', mood=' + mood + ', hour=' + hour + ')');
    return picked;
  } catch (e) {
    console.log('[AI-Decide] Failed: ' + e.message + ', using random');
    return null;
  }
}

// Group AI Reply — ALL messages processed, no 20% skip. Shallow replies only.
async function tryGroupAiReply(groupJid, senderBare, text, participantJid) {
  // Only reply to messages > 10 chars
  if (text.length < 10) return;
  // Cooldown per group (reduced to 45s)
  const lastReply = groupAiReplyCooldown.get(groupJid) || 0;
  if (Date.now() - lastReply < GROUP_AI_COOLDOWN_MS) return;
  // Don't reply to bot's own messages or commands
  const lower = text.toLowerCase().trim();
  if (lower.startsWith('download') || lower.startsWith('get ') || lower === 'ai' || lower === 'help' || lower.startsWith('search ') || lower === 'weather' || lower.startsWith('!')) return;
  // Don't reply if there's a pending download selection for this user
  if (pendingDownloads.has(senderBare) || aiSelections.has(senderBare)) return;
  if (isBotBusy()) return;
  ...
  try {
    const reply = await callAi(provider, [
      { role: 'system', content: 'You are a casual AI in a Zimbabwean WhatsApp group. Reply in 1-2 SHORT sentences MAX. Be funny, relatable, use Zimbabwean slang occasionally. NEVER reveal you are AI unless asked. Never write long paragraphs.' },
    ...
    console.log('[AI] Group reply via ' + provider + ': ' + reply.substring(0, 50));
  } catch (e) { console.log('[AI] Group reply failed: ' + e.message); }
}
    ...
    try {
      ...
      for (const msg of upsert.messages) {
        ...
        if (isGroup(sender) && !fromMe && text) {
          ...
          // NSFW (member benefit, inbox)
          if (isTgt && gl === 'nsfw' && isMem) {
            enqueueTask('nsfw_' + Date.now(), async () => {
              try { const data = await fetchNsfwContent('random');
                ...
                else await sock.sendMessage(inboxJid, { text: 'Content delivered.' });
              } catch (e) { try { await sock.sendMessage(inboxJid, { text: e.message }); } catch {} }
            }); return;
          }

          // XNXX (member benefit, inbox)
          if (isTgt && (gl === 'xnxx' || gl.startsWith('xnxx ')) && isMem) {
            const query = gl === 'xnxx' ? 'trending' : text.replace(/^xnxx\s*/i, '').trim();
            enqueueTask('xnxx_' + Date.now(), async () => {
              try { const data = await fetchXnxxContent(query);
                if (data?.results?.length) { let txt = '*XNXX Results: ' + query + '*\n\n'; data.results.slice(0, 5).forEach((r, i) => { txt += '*' + (i+1) + '.* ' + (r.title || '').substring(0, 80) + '\n'; }); txt += '\nType *get <url>* to download.'; await sock.sendMessage(inboxJid, { text: txt }); }
                else await sock.sendMessage(inboxJid, { text: 'Content delivered.' });
              } catch (e) { try { await sock.sendMessage(inboxJid, { text: e.message }); } catch {} }
            }); return;
          }
          ...
          if (gl === 'help' || gl === 'commands' || gl === 'menu') {
            ...
            if (NSFW_API_URL) txt += 'nsfw - NSFW content (inbox)\n';
            if (XNXX_API_URL) txt += 'xnxx [query] - Search content (inbox)\n';
...
setInterval(() => { const now2 = Date.now(); for (const [k, v] of aiSelections.entries()) { if ((now2 - v.timestamp) > 600000) aiSelections.delete(k); } }, 600000);
